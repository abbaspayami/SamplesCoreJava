public class Zalando_Task_2 {
    ...
    public static String solution(int AA, int AB, int BB) {
        ...
        while (AA > 0 || AB > 0 || BB > 0){
            ...
            if (AA == 0 && BB > 0 && !isBTheLastCharacter(sb)) {
                sb.append("BB");
                BB--;
            } else if (AB > 0) {
                sb.append("AB");
                AB--;
            } else if (AA > 0 && !isATheLastCharacter(sb)) {
                sb.append("AA");
                AA--;
            } else if (BB > 0 && !isBTheLastCharacter(sb)) {
                sb.append("BB");
                BB--;
            } else if (isATheLastCharacter(sb)) {
                AA--;
            } else if (isBTheLastCharacter(sb)) {
                BB--;
            }
        }
    ...
    private static boolean isATheLastCharacter(StringBuilder sb) {
        return sb.length() >= 1 && sb.charAt(sb.length() - 1) == 'A';
    }

    private static boolean isBTheLastCharacter(StringBuilder sb) {
        return sb.length() >= 1 && sb.charAt(sb.length() - 1) == 'B';
    }
...
Note that it is not possible to add another "AA" string as the result

would then contain "AAA".

Write a function:

class Solution { public String solution(int AA, int AB,
...
3. Given AA = 0, AB = 2 and BB = 0, the function should return
...
AA, AB and BB are integers within the range [0..10]: the resulting string will not be empty.
